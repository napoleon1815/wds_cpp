...
private: // the default access control of class is private
  ...
  // nonstatic member data for the object
  // access the nonstatic data need instant object fist
  char *name;
  int age;
  // static member data for the class
  // access the static data need initialization without class fist
  // and when initialization do not need static but must need type
  static int cnt;
public: // the public of access control means it can be accessed out ot class 

  // for every class will default provide three functions, so does basic type
  ...
  // without object only static function can be called 
  // and static function can only access static member data
 ...
 public:
   ...
   // the name after : is meaningless, the args after : will by the order of
   // private member to transfer args to the corresponding constructor
